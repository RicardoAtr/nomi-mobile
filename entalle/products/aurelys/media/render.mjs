...
await p.goto('file://' + process.cwd() + '/creativos.html', { waitUntil: 'networkidle' }); await p.evaluate(() => document.fonts.ready);
for (const id of ['s01','s02','s04','s05','s12']) await (await p.$('#' + id)).screenshot({ path: `aurelys-${id.slice(1)}.png` });
await b.close();
