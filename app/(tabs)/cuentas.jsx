...
const needsBanco    = ['corriente','vista','ahorro','tarjeta_credito','linea_credito','prestamo']
...
export default function Cuentas() {
  ...
  const resetForm = () => { setForm(defaultForm()); setShowModal(false) }
  ...
  const handleToggle = async (id, is_active) => {
    Alert.alert(
      ...
      is_active ? 'La cuenta no aparecerá en los cálculos.' : 'La cuenta volverá a aparecer.',
      [
  ...
  const setTipo = (key) => {
    ...
    setForm(f => ({ ...defaultForm(), tipo: key, icono: t?.icono ?? '🏦', color: f.color }))
  }
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={s.header}>
          <View>
            ...
            <Text style={s.subtitle}>{accounts.length} cuenta{accounts.length !== 1 ? 's' : ''} activa{accounts.length !== 1 ? 's' : ''}</Text>
          </View>
        ...
        <View style={s.listWrap}>
          ...
          {!loading && accounts.length === 0 && (
            <View style={s.emptyWrap}>
              ...
              <Text style={s.empty}>Agrega tu primera cuenta para empezar a registrar movimientos</Text>
          ...
          {accounts.map(a => {
            ...
            return (
              <TouchableOpacity key={a.id}
                style={[s.accountCard, { borderLeftColor: a.color, borderLeftWidth: 4 }]}
                  ...
                  {limite && (
                    <View style={s.limiteWrap}>
                      <View style={s.limiteLabelRow}>
                        ...
                        <Text style={[s.limitePct, { color: semaforo }]}>{pctUsado.toFixed(0)}%</Text>
                      </View>
                      <View style={s.barBg}>
                  ...
                  {!limite && (
                    <View style={s.accountFooter}>
                      <View>
                        ...
                        <Text style={[s.saldoVal, { color: Number(a.saldo_actual) >= 0 ? COLORS.text : COLORS.red }]}>
                          {clp(a.saldo_actual)}
                        </Text>
                      </View>
                      <View style={{ alignItems: 'flex-end' }}>
                        <Text style={s.saldoLabel}>Saldo inicial</Text>
                        <Text style={s.saldoInicialVal}>{clp(a.saldo_inicial)}</Text>
                      </View>
                    </View>
                  )}
                </View>
        ...
        <Text style={s.hint}>Mantén presionada una cuenta para activarla o desactivarla</Text>
          ...
          <View style={s.modalSheet}>
            ...
            <View style={s.modalHeader}>
              <View>
                ...
                {tipoActual && <Text style={s.modalSubtitle}>{tipoActual.desc}</Text>}
              </View>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 18 }}>
                {TIPOS.map(t => (
                  <TouchableOpacity key={t.key} onPress={() => setTipo(t.key)}
                    ...
                    <Text style={[s.tipoChipText, form.tipo === t.key && s.tipoChipTextActive]}>
                      {t.label}
                    </Text>
                  </TouchableOpacity>
                ))}
              ...
              {needsBanco.includes(form.tipo) && (
                <>
                  <Text style={s.inputLabel}>
                    {form.tipo === 'inversion' ? 'Institución *' : 'Banco / Institución *'}
                  </Text>
                  <TouchableOpacity style={s.selectBtn} onPress={() => setShowBancoModal(true)}>
                    <Text style={[s.selectBtnText, !form.banco && { color: COLORS.textSub }]}>
                  ...
                  {form.banco === 'Otro' && (
                    <TextInput style={s.modalInput} placeholder="Escribe el nombre..."
                      value={form.banco_otro} onChangeText={v => setForm(f => ({ ...f, banco_otro: v }))}
                      placeholderTextColor={COLORS.textSub} />
                  )}
                </>
              )}
              ...
              <TextInput style={s.modalInput}
                ...
                value={form.saldo_inicial} onChangeText={v => setForm(f => ({ ...f, saldo_inicial: numericOnly(v) }))}
                placeholderTextColor={COLORS.textSub} />
              ...
              {needsLimite.includes(form.tipo) && (
                <>
                  ...
                  <TextInput style={s.modalInput} placeholder="Ej: 1.000.000"
              ...
              {needsCierre.includes(form.tipo) && (
                <>
                  ...
                  <TextInput style={s.modalInput} placeholder="Ej: 15"
              ...
              {needsCuota.includes(form.tipo) && (
                <>
                  ...
                  <TextInput style={s.modalInput} placeholder="Ej: 150.000"
              ...
              <TouchableOpacity style={[s.saveBtn, { backgroundColor: form.color }]}
                ...
                <Text style={s.saveBtnText}>{saving ? 'Guardando...' : `Crear ${tipoActual?.label ?? 'cuenta'}`}</Text>
              </TouchableOpacity>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              {BANCOS_CL.map(b => (
                <TouchableOpacity key={b} style={s.bancoRow}
                  onPress={() => { setForm(f => ({ ...f, banco: b, banco_otro: '' })); setShowBancoModal(false) }}>
                  <Text style={[s.bancoText, form.banco === b && { color: COLORS.brand, fontWeight: '700' }]}>{b}</Text>
...
const s = StyleSheet.create({
  ...
  patrimonioCard:    { marginHorizontal: 20, borderRadius: 24, backgroundColor: COLORS.brand,
                       padding: 24, marginBottom: 20, overflow: 'hidden', minHeight: 120 },
  patrimonioLabel:   { color: 'rgba(255,255,255,0.75)', fontSize: 13, fontWeight: '500', marginBottom: 8 },
  patrimonioVal:     { color: '#fff', fontSize: 32, fontWeight: '800', letterSpacing: -1, marginBottom: 6 },
  patrimonioSub:     { color: 'rgba(255,255,255,0.65)', fontSize: 12 },
  circle1:           { position: 'absolute', width: 160, height: 160, borderRadius: 80,
                       backgroundColor: 'rgba(255,255,255,0.07)', right: -40, top: -40 },
  circle2:           { position: 'absolute', width: 100, height: 100, borderRadius: 50,
                       backgroundColor: 'rgba(255,255,255,0.07)', right: 50, bottom: -35 },
  listWrap:          { paddingHorizontal: 20 },
  emptyWrap:         { alignItems: 'center', paddingVertical: 50 },
  emptyEmoji:        { fontSize: 48, marginBottom: 12 },
  emptyTitle:        { fontSize: 18, fontWeight: '700', color: COLORS.text, marginBottom: 6 },
  empty:             { textAlign: 'center', color: COLORS.textSub, fontSize: 14,
  ...
  accountCard:       { backgroundColor: '#fff', borderRadius: 18, padding: 16, marginBottom: 12,
                       flexDirection: 'row', alignItems: 'flex-start', gap: 14 },
  accountIconWrap:   { width: 52, height: 52, borderRadius: 16, alignItems: 'center', justifyContent: 'center' },
  accountInfo:       { flex: 1 },
  ...
  saldoLabel:        { fontSize: 10, color: COLORS.textSub, fontWeight: '600',
  ...
  limiteLabelRow:    { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 6 },
  limiteLabel:       { fontSize: 11, color: COLORS.textSub, fontWeight: '600',
                       textTransform: 'uppercase', letterSpacing: 0.4 },
  limitePct:         { fontSize: 12, fontWeight: '800' },
  barBg:             { height: 7, backgroundColor: '#F4F6F9', borderRadius: 4, overflow: 'hidden', marginBottom: 6 },
  barFill:           { height: '100%', borderRadius: 4 },
  limiteRow:         { flexDirection: 'row', justifyContent: 'space-between' },
  limiteText:        { fontSize: 11, color: COLORS.textSub },
  hint:              { textAlign: 'center', fontSize: 11, color: COLORS.textSub,
                       fontStyle: 'italic', marginTop: 4, paddingHorizontal: 20 },
  ...
  inputLabel:        { fontSize: 12, fontWeight: '600', color: COLORS.textSub, marginBottom: 6,
                       marginTop: 4 },
